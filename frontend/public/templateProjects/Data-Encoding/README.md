...
Every file on your computer, every photo, every message, every web page is stored as sequences of 0s and 1s. In this project, you'll implement the conversion functions that connect those raw bits to the numbers, text, and colors humans actually work with.
...
In decimal, each position is worth ten times the position to its right (ones, tens, hundreds...). In binary, each position is worth **two times** the position to its right:
...
Binary is exact, but it's tedious to read. The number 255 in binary is `11111111`, eight digits. Hexadecimal (base 16) is a convenient shorthand: every group of four binary digits maps to exactly one hex digit.
...
Computers store text by mapping each character to a number. The most common mapping for standard English characters is ASCII (American Standard Code for Information Interchange), which assigns a value 0-127 to each letter, digit, and punctuation mark.
...
When your program saves the word `"Hello"`, it actually saves the five numbers `[72, 101, 108, 108, 111]`. Everything else (fonts, layout, rendering) is handled by other layers of software on top of that.
...
That is why image formats like JPEG and PNG use compression algorithms to reduce file sizes.
...
**Hints:**
- Python's built-in `ord()` gives the code for a single character
...
- Build and join the characters

### TODO #7: `rgb_to_hex(r, g, b)`

Convert RGB values to a hex color string like `"#FFA500"`.
...
Write a function that takes a string and returns a space-separated string of ASCII codes, then write the decoder that reverses it.
...
Run-length encoding (RLE) is one of the simplest compression schemes. Consecutive repeated characters are stored as a count and a character:
...
Create a small "image file" format. Write a text file where each line contains comma-separated RGB values:
...
- [ ] All functions handle the zero/empty-input edge cases
...
- Verify that `HEX_DIGITS` contains uppercase letters and that you index into it with the remainder
