...
In this project you learn how real-world systems protect passwords, why weak passwords fail even when hashed, and what trade-offs engineers make when choosing a security approach.
...
- What a hash function does and why it is a one-way operation
...
Right-click the `Password-Security` folder in the file explorer on the left and select **Open in Terminal**. This executes `cd` (change directory) in your terminal to the project folder so the commands below will work.
...
- Why plaintext storage is dangerous and what a database breach exposes
- What a hash function does and its three key properties
- How dictionary attacks work even when passwords are hashed
- How salting defeats rainbow tables
- A comparison of security approaches and their trade-offs
- Real-world case studies, the six functions you will implement, extension challenges, and troubleshooting

## Background: Why Password Storage Matters

When you create an account on a website, the site needs to verify your password on future logins. The naive approach is to store the password exactly as typed. This is called **plaintext storage**, and it is a serious security failure.

### The Database Breach Problem

Every database can potentially be stolen: through SQL injection, a misconfigured server, a disgruntled employee, or a compromised backup. When a database leaks, the attacker has a list of usernames paired with whatever the site stored for each password.
...
**One-way.** Given a hash, you cannot compute the original input. There is no "unhash" operation. This is what makes hashing useful for passwords: the database stores the hash, and even someone reading the database cannot directly recover the passwords.
...
- **Standard.** It is used in TLS, code signing, Bitcoin, and many other systems.

For general-purpose hashing and for learning, SHA-256 is the right starting point. Password storage has additional requirements beyond basic SHA-256; the extension challenges cover them.
...
- **Salting** adds a small amount of storage and a slightly more complex login flow, but is invisible to users.
- **bcrypt** intentionally runs slowly (the work factor is configurable), which adds a few hundred milliseconds to login but makes large-scale cracking impractical.
...
There is no single right answer. A banking application warrants different trade-offs than a school library portal. Part of security engineering is choosing measures appropriate to the value of what is being protected and the realistic threat landscape.
...
**What went wrong:** no salting, SHA-1 (even weaker than SHA-256), and no detection of the breach for years.

**What should have happened:** unique salts per user, a slow hashing algorithm such as bcrypt, and prompt breach detection and user notification.
...
- Concatenate the strings, then pass them to `hash_password()`.
...
Implement the functions in order. `verify_password` depends on `hash_password`; `dictionary_attack` depends on both; `hash_with_salt` and `dictionary_attack` together demonstrate the value of salting.
...
- How would you upgrade a system from SHA-256+salt to bcrypt without requiring all users to reset their passwords?
...
You are probably calling `.digest()` instead of `.hexdigest()`. The digest is raw bytes; the hexdigest is the hex-encoded string.
...
### `check_password_strength` Returns the Wrong Score
