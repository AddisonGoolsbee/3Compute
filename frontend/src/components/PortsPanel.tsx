  ...
  return (
    <RDialog.Root open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <RDialog.Portal>
        <RDialog.Overlay
          className={cn(
            'fixed inset-0 z-50 bg-ink-strong/40 transition-opacity duration-300',
        ...
        <RDialog.Content
          ...
          className={cn(
            'fixed top-0 right-0 z-50 h-full w-full max-w-sm bg-paper-elevated border-l border-rule-soft flex flex-col overflow-hidden shadow-lg focus:outline-none',
            'data-[state=open]:animate-in data-[state=closed]:animate-out',
            'data-[state=open]:slide-in-from-right data-[state=closed]:slide-out-to-right',
          )}
        >
          <RDialog.Title id={titleId} className="sr-only">Public URLs</RDialog.Title>
          <RDialog.Description className="sr-only">Claim and manage public subdomain mappings for your container ports.</RDialog.Description>
          {/* Header */}
          <div className="px-6 py-4 border-b border-rule-soft flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
              <Globe size={18} className="text-navy" aria-hidden="true" />
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  <label htmlFor={portInputId} className="text-xs text-ink-muted mb-1 block font-medium">Port</label>
                  <input
                    id={portInputId}
                    type="number"
                ...
                <div>
                  <label htmlFor={subdomainInputId} className="text-xs text-ink-muted mb-1 block font-medium">Subdomain</label>
