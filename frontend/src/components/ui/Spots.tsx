...
export function SpotLesson({ size = 220 }: SpotProps) {
  return (
...
export function SpotGradebook({ size = 220 }: SpotProps) {
  const rows = [
    { color: '#1f4e79', name: 60, score: 48, scoreColor: '#2d6a4f' }, // navy avatar, full pass
    { color: '#2d6a4f', name: 55, score: 50, scoreColor: '#2d6a4f' }, // forest, full
    { color: '#e09733', name: 65, score: 32, scoreColor: '#e09733' }, // ochre, partial
    { color: '#6d3aed', name: 50, score: 40, scoreColor: '#2d6a4f' }, // plum, full
    { color: '#e85d3f', name: 58, score: 50, scoreColor: '#2d6a4f' }, // tomato, full
  ];
  return (
    <svg width={size} height={size} viewBox="0 0 240 240" fill="none">
      <rect x="35" y="55" width="170" height="135" rx="8" fill="#fff" stroke="#d8cfb8" strokeWidth="1.5" />
      <rect x="35" y="55" width="170" height="22" rx="8" fill="#fbe1da" />
      <rect x="35" y="69" width="170" height="8" fill="#fbe1da" />
      <text x="48" y="71" fill="#e85d3f" style={{ font: '600 9px DM Sans, sans-serif' }}>Class · 7 students</text>
      {rows.map((r, i) => {
        const y = 90 + i * 18;
        return (
          <g key={i}>
            <circle cx="50" cy={y} r="6" fill={r.color} />
            <rect x="62" y={y - 3} width={r.name} height="3" rx="1.5" fill="#b8b4a8" />
            <rect x="140" y={y - 3} width="50" height="5" rx="2.5" fill="#ebe4d2" />
            <rect x="140" y={y - 3} width={r.score} height="5" rx="2.5" fill={r.scoreColor} />
          </g>
        );
      })}
    </svg>
...
export function SpotClassroom({ size = 220 }: SpotProps) {
...
export function SpotEditor({ size = 220 }: SpotProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 240 240" fill="none">
...
export function SpotGlobe({ size = 180 }: SpotProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 240 240" fill="none">
