...
class TestTerminalDockerIntegration:
    ...
    def test_full_terminal_connection_flow(
        self, mock_popen, mock_pty, mock_running, mock_flask_dependencies
    ):
    ...
    def test_container_spawning_integration(self, mock_flask_dependencies):
        ...
        with (
            ...
            assert (
                terminal.user_containers[999]["container_name"] == "user-container-999"
            )
    ...
    def test_multiple_users_data_persistence(self, temp_file):
        ...
        with patch("backend.auth.USERS_JSON_FILE", temp_file):
            ...
            for i, user in enumerate(users):
                update_user_data(
                    user["id"], {"email": user["email"]}, "192.168.1.1", 8000 + i * 10
                )
...
class TestFullWorkflowIntegration:
    ...
    def test_complete_user_workflow(
        self, mock_popen, mock_pty, mock_run, mock_flask_dependencies
    ):
        """Test complete user workflow from auth to terminal"""
        import backend.terminal as terminal
        from backend.terminal import handle_connect, handle_disconnect, handle_pty_input
    ...
    def test_multi_tab_user_session(self, mock_flask_dependencies):
        ...
        with (
            ...
            assert (
                terminal.session_map["tab1-session"]["container_name"]
                == terminal.session_map["tab2-session"]["container_name"]
            )  # Same user, same container
            assert (
                terminal.session_map["tab1-session"]["tab_id"]
                != terminal.session_map["tab2-session"]["tab_id"]
            )
...
class TestErrorHandlingIntegration:
    ...
    def test_docker_failure_handling(self, mock_flask_dependencies):
        ...
        with (
            ...
            patch("subprocess.run") as mock_subprocess,
            patch(
                "backend.terminal.spawn_container",
            ...
            assert (
                len(terminal.session_map) <= 1
            )  # Session may or may not be created depending on failure timing
