...
class TestTerminalModule:
    ...
    def test_set_winsize(self):
        ...
        with patch("fcntl.ioctl") as mock_ioctl, patch("struct.pack") as mock_pack:
    ...
    def test_handle_connect_new_user(
        self, mock_socketio, mock_attach, mock_flask_dependencies
    ):
        ...
        with (
            patch("backend.terminal.spawn_container") as mock_spawn,
    ...
    @patch("backend.terminal.attach_to_container")
    def test_handle_resize(
        self, mock_attach, mock_set_winsize, mock_flask_dependencies
    ):
...
class TestTabSessionManagement:
    ...
    def test_unique_tab_sessions(self):
        ...
        assert (
            terminal.session_map[tab1_sid]["fd"] != terminal.session_map[tab2_sid]["fd"]
        )
